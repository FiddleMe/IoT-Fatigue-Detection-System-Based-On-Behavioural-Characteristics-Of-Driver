...
def main():
    ...
    while validFrames < dummyFrames:
        ...
        if landmarks == 0:
            ...
            # cv2.putText(frame, "Calibration in Progress", (200, 30), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255), 1,
            #             cv2.LINE_AA)
            # cv2.imshow("Blink Detection Demo", frame)
    ...
    if __name__ == "__main__":
        ...
        while 1:
            try:
                ...
                for i in range(0, len(leftEyeIndex)):
                    cv2.circle(frame, (landmarks[leftEyeIndex[i]][0], landmarks[leftEyeIndex[i]][1]), 1, (0, 0, 255),
                            -1, lineType=cv2.LINE_AA)

                for i in range(0, len(rightEyeIndex)):
                    cv2.circle(frame, (landmarks[rightEyeIndex[i]][0], landmarks[rightEyeIndex[i]][1]), 1, (0, 0, 255),
                            -1, lineType=cv2.LINE_AA)

                if drowsy:
                    cv2.putText(frame, "! ! ! DROWSINESS ALERT ! ! !", (70, 50), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2,
                                cv2.LINE_AA)
                    print("Drowsy!")  # Print the drowsy alert instead of sound alert
                    break

                else:
                    cv2.putText(frame, "Blinks : {}".format(blinkCount), (460, 80), cv2.FONT_HERSHEY_COMPLEX, 0.8, (0, 0, 255),
                                2, cv2.LINE_AA)
                    # (0, 400)
